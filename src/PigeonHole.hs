...
approxPracticalWith atMost vScope paramCNF xIns =
  let xIns' = xIns ++ replicate nFalse (True, vScope $ V "false") ++ replicate nTrue (True, vScope $ V "true")
      ...
      cnfFix = map (\i -> [(False, snd $ xIns' !! (i-1))]) [nT-nTrue-nFalse+1..nT-nTrue]
          ++ map (\i -> [xIns' !! (i-1)]) [nT-nTrue+1..nT]
  in  cnfTop ++ cnfP ++ cnfX ++ cnfFix

pigeonHole :: Bool -> (Int, Int) -> IO CNF
pigeonHole blDebug (l, m) = do
  let n = l * m
  (_, paramCNF) <- theBestEfficiency False True (l, l*m)
  putStrLn $ "Using parameterCNF: " ++ show paramCNF
  ...
  let holeAM = concatMap (\j -> approx (Scope $ "holeAM" ++ show j) $ [ lV i j | i <- [1..n] ]) [1..m]
      holeAMdebug = counter (Scope "holeAMdebug") [ lV i 1 | i <- [1..n] ] (l - 1)
      pigeonAM = concatMap (\i ->  (counter (Scope $ "pigeonAM" ++ show i) [ lV i j | j <- [1..m] ] 1)) [1..n]
      pigeonAL = map (\i -> [ lV i j | j <- [1..m] ]) [1..n]
  pure $ holeAM ++ (if blDebug then holeAMdebug else []) ++ pigeonAM ++ pigeonAL
  -- > pigeonHole (2, 3) >>= printCNF
  -- > pigeonHole (2, 3) >>= printDIMACS
  -- > pigeonHole (2, 3) >>= \cnf -> generateDIMACSwithTrue cnf []
  -- (3, 4) -> SAT, (3, 5) -> UNSAT !!
